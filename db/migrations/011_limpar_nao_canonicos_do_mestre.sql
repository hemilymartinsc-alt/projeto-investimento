-- Remove do cadastro mestre operacional variantes B3 legadas. A execução é
-- idempotente: no ambiente já saneado, o DELETE afeta zero linhas.
create temporary table tmp_ativos_nao_canonicos_candidatos
on commit drop
as
select id
from investimento.ativos
where instrumento_canonico = false
  and elegivel_analise = false;

create unique index tmp_ativos_nao_canonicos_candidatos_id_idx
    on tmp_ativos_nao_canonicos_candidatos (id);
...
declare
    ...
    removidos bigint;
begin
    -- O bloqueio é restrito aos candidatos e dura somente durante a auditoria
    -- de dependências e o DELETE final.
    perform 1
    from investimento.ativos master
    join tmp_ativos_nao_canonicos_candidatos candidato
      on candidato.id = master.id
    for update of master;

    select count(*)
      into candidatos_iniciais
      from tmp_ativos_nao_canonicos_candidatos;

    -- Protege qualquer ativo referenciado por FK contra investimento.ativos,
    -- inclusive FKs compostas ou referências a uma chave diferente de id.
    for fk in
        select
    ...
    loop
        execute format(
            $delete_candidates$
            delete from tmp_ativos_nao_canonicos_candidatos candidato
    ...
    -- usam ativo_id ou id_ativo com o mesmo tipo da chave do cadastro mestre.
    ...
    using tmp_ativos_nao_canonicos_candidatos candidato
    where master.id = candidato.id
      and master.instrumento_canonico = false
      and master.elegivel_analise = false;

    get diagnostics removidos = row_count;

    raise notice
        'Limpeza não canônicos: candidatos_iniciais=%, protegidos_por_dependencia=%, removidos=%',
        candidatos_iniciais,
        protegidos_por_dependencia,
        removidos;
end
